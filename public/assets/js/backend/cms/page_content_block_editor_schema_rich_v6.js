define(['jquery','backend/cms/icon_picker'], function ($, IconPicker) {
    ...
    var applyOneItem = function (item, schema, page) {
        ...
        // label_capability PC description is rendered by the dedicated Rich V3
        // editor in the server form. Do not create a second generic rich editor.
        if (page === 'label' && String(schema.label || '') === '能力图文') {
            inlineColorFields = {};
        }
        item.find('[data-' + page + '-field]').each(function () {
            ...
            } else if (input.is('textarea') && !input.is('[data-label-capability-rich-source]')) {
    ...
    var applyItems = function (editor, schema, page) {
        ...
        var scope = schema.item_scope === 'form' ? form : editor;
        ...
        var sectionBody = editor.find('[data-structured-section-body="items"]').first();
        ...
        var itemSelector = '[data-' + page + '-item]';
        scope.find(itemSelector).each(function (index) {
            applyOneItem(this, schema, page);
            updateItemHeading(this, schema, page, index);
        });

        if (schema.hide_item_section) {
            section.hide();
            sectionBody.hide();
        }
    };
    ...
    var reorderSections = function (editor, schema) {
        var order = schema && $.isArray(schema.section_order) ? schema.section_order : [];
        if (!order.length) return;
        var anchor = null;
        $.each(order, function (_, name) {
            var header = editor.find('[data-structured-section="' + name + '"]').first();
            var body = editor.find('[data-structured-section-body="' + name + '"]').first();
            if (!header.length || !body.length) return;
            if (anchor) {
                header.insertAfter(anchor);
                body.insertAfter(header);
            } else {
                var firstSection = editor.find('[data-structured-section]').first();
                if (firstSection.length && firstSection.get(0) !== header.get(0)) {
                    header.insertBefore(firstSection);
                    body.insertAfter(header);
                }
            }
            anchor = body;
    ...
    var refreshSections = function (form) {
        form.find('[data-structured-section]').each(function () {
            ...
            if (name === 'items') {
                ...
                if (schemas[blockKey] && schemas[blockKey].hide_item_section) {
                    header.hide();
                    body.hide();
                    return;
                }
    ...
    var apply = function (form) {
        ...
        $.each(PAGE_EDITORS, function (page, selector) {
            ...
            var specificHeading = editor.find('[data-structured-section="specific"] h4').first();
