define(['jquery'], function ($) {
    ...
    var buildUi = function (field) {
        ...
        var role = String(field.attr('data-label-capability-rich-role') || (String(source.attr('data-label-field') || '') === 'mobile_text' ? 'mobile' : 'pc'));
        var footerText = role === 'mobile'
            ? '移动端富文本编辑：功能与 PC 说明一致；留空时前台继续继承 PC 说明。'
            : 'PC 富文本编辑：选中部分文字后，可直接设置加粗、斜体、删除线或文字颜色。';

        wrapper
            .addClass('cms-label-capability-richbox')
            .attr('data-label-capability-rich-editor-role', role)
            .empty()
            .append(
                ...
                '<div class="cms-label-capability-rich-footer">' + footerText + '</div>'
            );
    ...
    var initField = function (field) {
        field = $(field);
        var source = field.find('[data-label-capability-rich-source]').first();
        if (!source.length || source.data('label-capability-rich-v3-ready')) return;
        var editor = buildUi(field);
        if (!editor.length) return;

        source.data('label-capability-rich-v3-ready', true);
    ...
    var bind = function (form) {
        ...
        form.on('mousedown.labelCapabilityRichV3', '[data-label-capability-command],[data-label-capability-clear-format]', function (event) {
            event.preventDefault();
            var field = $(this).closest(FIELD);
            saveSelection(field.find('[data-label-capability-rich-editor]').first());
        });

        form.on('click.labelCapabilityRichV3', '[data-label-capability-command]', function () {
            var button = $(this);
            execCommand(button.closest(FIELD), button.attr('data-label-capability-command'));
        });

        form.on('mousedown.labelCapabilityRichV3', '[data-label-capability-rich-color]', function () {
            var field = $(this).closest(FIELD);
            saveSelection(field.find('[data-label-capability-rich-editor]').first());
        });

        form.on('change.labelCapabilityRichV3', '[data-label-capability-rich-color]', function () {
            var input = $(this);
            var color = normalizeColor(input.val()) || '#e25042';
            execCommand(input.closest(FIELD), 'foreColor', color);
        });

        form.on('click.labelCapabilityRichV3', '[data-label-capability-clear-format]', function () {
