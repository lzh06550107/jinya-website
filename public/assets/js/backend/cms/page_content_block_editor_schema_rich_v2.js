define(['jquery','backend/cms/icon_picker'], function ($, IconPicker) {
    ...
    var applyOneItem = function (item, schema, page) {
        ...
        // Cache-independent fallback for the labels capability editor. Even if
        // an older serialized schema reaches the browser, PC/mobile description
        // fields still become rich-text color editors in this v2 module.
        if (page === 'label' && String(schema.label || '') === '能力图文') {
            inlineColorFields.text = true;
            inlineColorFields.mobile_text = true;
        }
        item.find('[data-' + page + '-field]').each(function () {
