    ...
    public function _initialize()
    {
        ...
        if (!$this->auth->match($this->noNeedLogin)) {
            ...
            if (!$this->auth->isLogin()) {
                ...
                // 会话失效后统一回到后台登录页；登录成功后回后台首页，避免重新执行过期的 AJAX/POST 操作。
                $requestedUrl = Session::get('referer');
                $requestedUrl = $requestedUrl ? $requestedUrl : $this->request->url();
                $loginUrl = url('index/login', ['url' => url('index/index')]);

                // AJAX 请求返回机器可识别的登录失效标记，由后台全局 JS 将顶层窗口跳转到登录页。
                if (IS_AJAX) {
                    $this->error(
                        ...
                        ['login_required' => 1],
                        0,
                        ['X-Admin-Login-Required' => '1']
                    );
                }

                // 普通页面/iframe/dialog 请求不再渲染“请登录后操作”错误页，直接进入登录页。
                $this->redirect($loginUrl, [], 302, ['referer' => $requestedUrl]);
                exit;
            }
            // 判断是否需要验证权限
