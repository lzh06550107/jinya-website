...
{
    public function publishedTree(){ $rows=$this->rows(ProductCategory::where('status','normal')->order('weigh desc,id asc')->select());$by=[];foreach($rows as $r){$r['children']=[];$by[(int)$r['parent_id']][]=$r;}$build=function($id)use(&$build,&$by){$items=isset($by[$id])?$by[$id]:[];foreach($items as &$it)$it['children']=$build((int)$it['id']);unset($it);return $items;};return $build(0); }
    public function findPublishedBySlug($slug){return $this->row(ProductCategory::where('slug',$slug)->where('status','normal')->find());}
}
