...
use app\common\model\cms\ArticleCategory as CmsModel;
...
class ArticleCategory extends Backend
{
    ...
    public function index()
    {
        ...
        $contentCounts = [];
        foreach (\app\common\model\cms\Article::field('category_id,COUNT(*) AS total')->group('category_id')->select() as $stat) {
    ...
    public function del($ids = '')
    {
        ...
        if ($this->hasRelatedContent($idList)) {
            $this->error('该分类存在新闻或子分类，请先移动内容或停用分类');
        }
    ...
    protected function hasRelatedContent(array $idList)
    {
        ...
        return \app\common\model\cms\Article::where('category_id', 'in', $idList)->count() > 0;
    }
